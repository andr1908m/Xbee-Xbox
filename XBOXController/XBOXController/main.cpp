...
int main(int argc, char* argv[])
{
	...
	/*COM8 refers to the port that your XBEE router is connected to. To find this out you need to use XCTU. Ask Zotto to clarify in person*/
	hSerial = CreateFile("COM7", GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);// dont need to GENERIC _ WRITE changed to com4 on laptop
			...
			while (true)
			{
																	  ...
																	  and send 2 bytes for each case i.e. make char_L and char_R = 'L'/'R' respectively, 													  send that with coordinate data to specify which wheel*/
				{
					...
					printf("%d\t%d\t%d\t%d\t%d\t%d\n",
						...
						gpState.Gamepad.sThumbRY / 256,
						gpState.Gamepad.bLeftTrigger,
						gpState.Gamepad.bRightTrigger);// Thumbstick values are divided by 256 for better consistency
				...
				//put the buttons outside the if statements so they print without having to move sticks
				unsigned char highByte = gpState.Gamepad.wButtons/256;
				unsigned char lowByte = gpState.Gamepad.wButtons%256;
				printf("%d\t%d", highByte,lowByte);
				
				WriteFile(hSerial, &highByte, 1, &dwBytesWritten, NULL);
				WriteFile(hSerial, &lowByte, 1, &dwBytesWritten, NULL);
			}
		}
	}
	delete(Player1);
